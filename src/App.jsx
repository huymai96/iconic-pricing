...
export default function App(){
  ...
  return (<div style={{maxWidth:1100,margin:"32px auto",padding:"0 16px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <div><button onClick={reset}>Reset</button> <button onClick={()=>window.print()}>Print / Save PDF</button></div>
    </div>
      ...
      <div style={{display:"grid",gap:12,gridTemplateColumns:"repeat(3,minmax(0,1fr))"}}>
        ...
        <div style={{gridColumn:"span 2"}}>
          <div style={{display:"grid",gap:8,gridTemplateColumns:"repeat(3,minmax(0,1fr))"}}>
            {[["sweatshirts","Sweatshirts +$0.30"],["sweatpants_ziphoodies","Sweatpants/Zip Hoodies +$0.30"],["tanks_polos_shorts","Tanks/Polos/Shorts +$0.22"],["polyester","Polyester +$0.22"],["aprons","Aprons +$0.22"],["towels_bandanas","Small Towels/Bandanas +$0.22"],["toteBags","Tote Bags +$0.22"]].map(([k,l])=>(<label key={k} style={{display:"flex",alignItems:"center",gap:6}}><input type="checkbox" checked={garmentType[k]} onChange={e=>setGarmentType(g=>({...g,[k]:e.target.checked}))}/> {l}</label>))}
          </div>
        </div>
      </div>
    </div>
    <div style={{border:"1px solid #e9eaeb",borderRadius:12,padding:16,marginTop:12}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><h3 style={{margin:0}}>Print Locations</h3><button onClick={addLocation}>+ Add Location</button></div>
      {locations.map(loc=>(<div key={loc.id} style={{display:"grid",gap:12,gridTemplateColumns:"repeat(12,minmax(0,1fr))",alignItems:"end",border:"1px dashed #e5e7eb",padding:12,borderRadius:12,marginTop:10}}>
        <div style={{gridColumn:"span 3"}}><div>Location Name</div><input value={loc.name} onChange={e=>setLocations(a=>a.map(l=>l.id===loc.id?{...l,name:e.target.value}:l))}/><div style={{fontSize:12,color:"#666"}}>Base ${pickPrice(loc.colors,isDark,qty).toFixed(2)} / item</div></div>
        <div style={{gridColumn:"span 2"}}><div>Colors</div><input type="number" min={1} max={12} value={loc.colors} onChange={e=>setLocations(a=>a.map(l=>l.id===loc.id?{...l,colors:Math.max(1,Math.min(12,parseInt(e.target.value||"0",10)))}:l))}/></div>
        <div style={{gridColumn:"span 2"}}><div>Sleeve?</div><select value={loc.isSleeve} onChange={e=>setLocations(a=>a.map(l=>l.id===loc.id?{...l,isSleeve:e.target.value}:l))}><option value="none">No</option><option value="short">Short (+$0.22)</option><option value="long">Long (+$0.25)</option></select></div>
        <div style={{gridColumn:"span 2"}}><div>Screens (optional)</div><input type="number" min={0} placeholder={String(loc.colors)} value={loc.screensOverride ?? ""} onChange={e=>setLocations(a=>a.map(l=>l.id===loc.id?{...l,screensOverride:e.target.value===""?null:parseInt(e.target.value||"0",10)}:l))}/><label style={{fontSize:12,color:"#666",display:"flex",alignItems:"center",gap:6,marginTop:6}}><input type="checkbox" checked={!!loc.reorderScreens} onChange={e=>setLocations(a=>a.map(l=>l.id===loc.id?{...l,reorderScreens:e.target.checked}:l))}/> Reorder screens ($10/screen)</label></div>
        <div style={{gridColumn:"span 3"}}>
          ...
          <div><div>Color Changes (count)</div><input type="number" min={0} value={loc.colorChanges} onChange={e=>setLocations(a=>a.map(l=>l.id===loc.id?{...l,colorChanges:Math.max(0,parseInt(e.target.value||"0",10))}:l))}/><div style={{fontSize:12,color:"#666"}}>$10 × colors × changes</div></div>
        </div>
        {locations.length>1 && <div style={{gridColumn:"span 12",display:"flex",justifyContent:"flex-end"}}><button onClick={()=>removeLocation(loc.id)} style={{color:"#c1121f"}}>Remove Location</button></div>}
      </div>))}
    </div>
    <div style={{border:"1px solid #e9eaeb",borderRadius:12,padding:16,marginTop:12,display:"grid",gap:12,gridTemplateColumns:"repeat(2,minmax(0,1fr))"}}>
      <div><h3>Personalization</h3><div style={{display:"grid",gap:10,gridTemplateColumns:"repeat(3,minmax(0,1fr))"}}>
      ...
      </div><div style={{fontSize:12,color:"#666"}}>Numbering $2/digit, Names $5/name.</div></div>
      <div><h3>Packaging & Handling</h3><div style={{display:"grid",gap:10,gridTemplateColumns:"repeat(3,minmax(0,1fr))"}}>
      ...
      </div><div style={{fontSize:12,color:"#666"}}>If Specialty Ink selected and total imprints &lt; 100, a flat $25 is added.</div></div>
    </div>
    <div style={{border:"1px solid #e9eaeb",borderRadius:12,padding:16,marginTop:12}}>
      <h3>Quote Summary</h3>
      <div style={{display:"grid",gap:12,gridTemplateColumns:"repeat(3,minmax(0,1fr))"}}>
        <div style={{border:"1px solid #e9eaeb",borderRadius:12,padding:12}}><div style={{color:"#666",fontSize:12}}>Per-Item Subtotal</div><div style={{fontSize:28,fontWeight:700}}>${'{'}computed.perItem.perItemSubtotal.toFixed(2){'}'}</div>
          <ul style={{fontSize:12,color:"#666"}}>
            ...
            { '{' }computed.perItem.specialtyPerItem > 0 && <li>Specialty ink: ${'{'}computed.perItem.specialtyPerItem.toFixed(2){'}'}</li>{ '}' }
            { '{' }computed.perItem.garmentPerItem > 0 && <li>Garment adds: ${'{'}computed.perItem.garmentPerItem.toFixed(2){'}'}</li>{ '}' }
          </ul>
        </div>
        <div style={{border:"1px solid #e9eaeb",borderRadius:12,padding:12}}><div style={{color:"#666",fontSize:12}}>Flat Adds</div><div style={{fontSize:28,fontWeight:700}}>${'{'}computed.flat.flatSubtotal.toFixed(2){'}'}</div>
          <ul style={{fontSize:12,color:"#666"}}>
        ...
        <div style={{border:"1px solid #e9eaeb",borderRadius:12,padding:12}}><div style={{color:"#666",fontSize:12}}>Totals</div><div style={{fontSize:28,fontWeight:700}}>${'{'}computed.grandTotal.toFixed(2){'}'}</div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              { '{' }computed.flat.screensDetail.map((row,i)=>(
                <tr key={ '{' }i{ '}' }><td>{ '{' }row.name{ '}' }</td><td>{ '{' }row.screenCount{ '}' }</td><td>${'{'}row.perScreen.toFixed(2){'}'}/screen</td><td>${'{'}row.total.toFixed(2){'}'}</td></tr>
              )){ '}' }
            </tbody>
          </table>
        </div>
